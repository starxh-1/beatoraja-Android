...
- 涉及 USB/物理键盘导致屏幕键盘误弹或物理键无输入时，先读
  `docs/usb-keyboard-onscreen-ime-debug.md`；游戏界面默认抑制 IME，但必须保持 libGDX
  的 `GLSurfaceView` 可聚焦/已聚焦，只清隐藏 `EditText`/IME 焦点，搜索框是主动进入文本输入态的例外。
