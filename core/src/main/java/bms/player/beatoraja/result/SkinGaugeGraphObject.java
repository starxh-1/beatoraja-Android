...
public class SkinGaugeGraphObject extends SkinObject {
	...
	public void prepare(long time, MainState state) {
		...
		if(shapetex != null && (shapetex.getTexture().getWidth() != (int) region.getWidth() || shapetex.getTexture().getHeight() != (int) region.getHeight())) {
			needRebuild = true;
		}
	...
	/**
	 * Rebuilds the gauge textures - creates new backtex and shapetex with the current
	 * gauge history. This is called from prepare() before rendering, so no I/O or
