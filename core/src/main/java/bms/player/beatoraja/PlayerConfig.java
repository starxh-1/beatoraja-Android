...
public final class PlayerConfig {
	...
	public static void init(Config config) {
		// TODO プレイヤーアカウント検証
		try {
			// 使用 java.io.File 处理绝对路径，在 Android/Desktop 上均可靠
			java.io.File playerDir = new java.io.File(config.getPlayerpath());
			if (!playerDir.exists()) {
				playerDir.mkdirs();
				com.badlogic.gdx.Gdx.app.log("PlayerConfig", "Created player directory: " + playerDir.getAbsolutePath());
			}
	...
	public static void create(String playerpath, String playerid) {
		try {
			if (com.badlogic.gdx.Gdx.app.getType() == com.badlogic.gdx.Application.ApplicationType.Android) {
				// Android 端路径是绝对路径，使用 absolute() 确保正确创建外部存储目录
				com.badlogic.gdx.files.FileHandle dir = com.badlogic.gdx.Gdx.files.absolute(playerpath + "/" + playerid);
				if (!dir.exists()) {
					dir.mkdirs();
				}
			} else {
			...
			com.badlogic.gdx.Gdx.app.error("PlayerConfig", "Failed to create PlayerConfig for: " + playerid, e);
		}
	}
	...
	public static String[] readAllPlayerID(String playerpath) {
		...
		try {
			if (com.badlogic.gdx.Gdx.app.getType() == com.badlogic.gdx.Application.ApplicationType.Android) {
				// Android 端 playerpath 是绝对路径，使用 absolute() 而非 local()
				com.badlogic.gdx.files.FileHandle dir = com.badlogic.gdx.Gdx.files.absolute(playerpath);
				if (dir.exists() && dir.isDirectory()) {
					...
					// 目录不存在时用 java.io.File 作为备选
					java.io.File dir2 = new java.io.File(playerpath);
					if (dir2.exists() && dir2.isDirectory()) {
						for (java.io.File child : dir2.listFiles()) {
							if (child.isDirectory()) l.add(child.getName());
						}
					}
				}
			} else {
			...
			com.badlogic.gdx.Gdx.app.error("PlayerConfig", "Failed to read all player IDs", e);
		}
			...
			if (com.badlogic.gdx.Gdx.app.getType() == com.badlogic.gdx.Application.ApplicationType.Android) {
				...
				if (fh.exists()) {
					try (Reader reader = new InputStreamReader(fh.read(), StandardCharsets.UTF_8)) {
						...
						// 尝试备份
						try {
							com.badlogic.gdx.files.FileHandle backup = com.badlogic.gdx.Gdx.files.absolute(playerpath + "/" + playerid + "/config_backup.json");
							backup.writeBytes(fh.readBytes(), false);
						} catch (Throwable e1) {
					...
					// 尝试旧配置文件路径
					com.badlogic.gdx.files.FileHandle fhOld = com.badlogic.gdx.Gdx.files.absolute(filePathOld);
					if (fhOld.exists()) {
...
//					e1.printStackTrace();
	...
	public static void write(String playerpath, PlayerConfig player) {
		if (playerpath == null || player == null || player.getId() == null) {
			return;
		}
		try {
			if (com.badlogic.gdx.Gdx.app.getType() == com.badlogic.gdx.Application.ApplicationType.Android) {
				...
				// 确保目录存在
				com.badlogic.gdx.files.FileHandle dirHandle = com.badlogic.gdx.Gdx.files.absolute(playerpath + "/" + player.getId());
				if (!dirHandle.exists()) {
					dirHandle.mkdirs();
				}
				...
				fh.writeString(json.prettyPrint(player), false, "UTF-8");
				com.badlogic.gdx.Gdx.app.log("PlayerConfig", "Successfully saved to: " + fh.path());
			} else {
				// 其他平台保持原样
			...
			com.badlogic.gdx.Gdx.app.error("PlayerConfig", "Failed to save PlayerConfig", e);
		} catch (Throwable t) {
			com.badlogic.gdx.Gdx.app.error("PlayerConfig", "Unexpected error when saving PlayerConfig", t);
		}
	}
