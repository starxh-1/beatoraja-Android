...
public class JSONSkinLoader extends SkinLoader {
	...
	public SkinHeader loadHeader(File p) {
		...
		try {
			...
			if (com.badlogic.gdx.Gdx.app.getType() == com.badlogic.gdx.Application.ApplicationType.Android) {
				...
				com.badlogic.gdx.files.FileHandle handle = com.badlogic.gdx.Gdx.files.absolute(p.toString());
				if (!handle.exists()) {
					// 如果absolute不存在，尝试internal（assets目录）
					handle = com.badlogic.gdx.Gdx.files.internal(p.toString());
				}
			...
			Logger.getGlobal().severe("JSONスキンファイルが見つかりませんまたは読み込みエラー : " + p.toString() + " - " + e.getMessage());
			e.printStackTrace();
		}
		return header;
	}
	...
	public Skin load(File p, SkinType type, SkinConfig.Property property) {
		...
		try {
			...
			Logger.getGlobal().severe("何らかの原因でJSONスキンファイルの読み込みに失敗しました : " + p.toString());
			e.printStackTrace();
		}
		return skin;
	}
		...
		try {
			...
			if (sk.destination != null) {
				...
				if (obj != null) {
					...
					// 纹理设置（默认 Linear）—— 所有皮肤对象一律升 Linear。
					// 必须在 setDestination 之后：dst.filter 声明此时已写入 dstfilter，
					// 详见 SkinTextureFilterPolicy 类注释。
					SkinTextureFilterPolicy.apply(obj);
					skin.add(obj);
				}
				}
			}
	...
	protected Object getSource(String srcid, File p) {
		...
		if(data == null) {
			Logger.getGlobal().warning("Source not found in sourceMap: srcid=" + srcid);
			return null;
		}
