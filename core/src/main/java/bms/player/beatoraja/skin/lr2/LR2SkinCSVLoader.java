	...
	public LR2SkinCSVLoader(Resolution src, Config c) {
		...
		addCommandWord(new CommandWord("INCLUDE") {
			...
			public void execute(String[] str) {
				...
				// Android 大小写敏感问题修复
				if (com.badlogic.gdx.Gdx.app.getType() == com.badlogic.gdx.Application.ApplicationType.Android && !handle.exists()) {
				...
				if (handle.exists()) {
					try (BufferedReader br = new BufferedReader(
						...
						java.util.logging.Logger.getGlobal().warning("INCLUDE failed: " + path + ", error: " + e.getMessage());
						e.printStackTrace();
					}
				} else {
					java.util.logging.Logger.getGlobal().warning("INCLUDE file not found: " + path);
				}
			}
		});
		...
		addCommandWord(new CommandWord("IMAGE") {
			...
			public void execute(String[] str) {
				...
				if (exists) {
					...
					if (isMovie) {
						try {
							imagelist.add(new SkinSourceMovie(actualFile.getAbsolutePath()));
						} catch (Throwable e) {
						...
						imagelist.add(SkinLoader.getTexture(actualFile.getPath(), usecim));
					}
				} else {
					Logger.getGlobal()
		...
		addCommandWord(new CommandWord("LR2FONT") {
			...
			public void execute(String[] str) {
				...
				if (exists) {
					...
					Logger.getGlobal()
							.warning("LR2FONT " + fontlist.size + " : ファイルが見つかりません : " + path);
					fontlist.add(null);
				}
				// System.out
				// .println("Image Loaded - " + (imagelist.size() -
			...
			public void execute(String[] str) {
				...
				if (gr >= 100) {
					...
					if (values[2] < imagelist.size && imagelist.get(values[2]) != null) {
						Object obj = imagelist.get(values[2]);
						if (obj instanceof SkinSourceMovie) {
							part = new SkinImage((SkinSourceMovie) obj);
						} else {
							part = new SkinImage(new SkinSourceImage(getSourceImage(values), values[10], values[9]));
						}
					}
				}
				if (part != null) {
		...
		addCommandWord(new CommandWord("SRC_SLIDER") {
			...
			public void execute(String[] str) {
				...
				TextureRegion[] images = getSourceImage(values);
				if (images != null) {
					SkinSource source = new SkinSourceImage(images, values[10], values[9]);
		...
		addCommandWord(new CommandWord("SRC_BUTTON") {
			...
			public void execute(String[] str) {
				button = null;
				int gr = Integer.parseInt(str[2]);
				if (gr < imagelist.size && imagelist.get(gr) instanceof Texture) {
					Texture tex = (Texture) imagelist.get(gr);
					int[] values = parseInt(str);
					int x = values[3];
					int y = values[4];
					int w = values[5];
	...
	protected TextureRegion[] getSourceImage(int[] values) {
		if (values[2] < imagelist.size && imagelist.get(values[2]) != null) {
			...
			if (obj instanceof Texture) {
				return getSourceImage((Texture) obj, values[3], values[4], values[5], values[6],
						values[7], values[8]);
			}
		}
