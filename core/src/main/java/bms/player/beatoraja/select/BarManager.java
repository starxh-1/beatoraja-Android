...
//		}
		...
		try {
			Json json = new Json();
			FileHandle fh = Gdx.app.getType() == Application.ApplicationType.Android
					? Gdx.files.local("folder/default.json")
					: Gdx.files.internal("folder/default.json");
			if(!fh.exists()) fh = Gdx.files.internal("folder/default.json");
			CommandFolder[] cf = json.fromJson(CommandFolder[].class, fh.read());
		...
		try {
			...
			FileHandle fh = Gdx.app.getType() == Application.ApplicationType.Android
					? Gdx.files.local("random/default.json")
					: Gdx.files.internal("random/default.json");
			if(!fh.exists()) fh = Gdx.files.internal("random/default.json");
	...
	public boolean updateBar(DirectoryBar bar) {
		...
		if (MainLoader.getIllegalSongCount() > 0) {
			...
			dir.clear();
			sourcebars.clear();
			// Avoid doing potentially blocking DB work directly on GLThread: use safe wrapper
			...
			if (select.resource.getConfig().isFolderlamp()) {
				...
				for (Bar b : l) {
					if (b instanceof bms.player.beatoraja.select.bar.DirectoryBar) {
						foldersToUpdate.add(b);
					}
				}
				if (foldersToUpdate.size > 0) {
					new Thread(() -> {
						for (Bar folderBar : foldersToUpdate) {
							try {
								((bms.player.beatoraja.select.bar.DirectoryBar) folderBar).updateFolderStatus();
							} catch (Exception e) {
								java.util.logging.Logger.getGlobal().warning("Error updating folder status: " + e.getMessage());
							}
						}
					}).start();
				}
			...
			// getChildren may access DB; protect GLThread by catching errors and returning empty
			l.addAll(safeGetChildren(bar));
	...
	private Bar[] safeGetChildren(Bar bar) {
		...
		try {
			if (bar instanceof bms.player.beatoraja.select.bar.DirectoryBar) {
				return ((bms.player.beatoraja.select.bar.DirectoryBar) bar).getChildren();
			}
		} catch (Throwable t) {
			Logger.getGlobal().warning("Error while getting children for bar '" + (bar != null ? bar.getTitle() : "null") + "': " + t.getMessage());
		}
		return new Bar[0];
	}
	...
	public void setSelectedPosition(float value) {
		if (currentsongs == null || value < 0 || value >= 1) {
			return;
		}
	...
	static final class BarContentsLoaderThread extends Thread {
		...
		public void run() {
			...
			for (Bar bar : bars) {
				...
				// 启用 folderlamp 更新，使用异步方式避免UI阻塞
				if (select.resource.getConfig().isFolderlamp()) {
					if (bar instanceof bms.player.beatoraja.select.bar.DirectoryBar) {
						final bms.player.beatoraja.select.bar.DirectoryBar dirBar = (bms.player.beatoraja.select.bar.DirectoryBar) bar;
						// 在后台线程更新文件夹状态
						new Thread(() -> {
							try {
								dirBar.updateFolderStatus();
							} catch (Exception e) {
								java.util.logging.Logger.getGlobal().warning("Error updating folder status: " + e.getMessage());
							}
						}).start();
