...
public class SkinNote extends SkinObject {
	...
	public SkinNote(SkinSource[] note, SkinSource[][] longnote, SkinSource[] minenote) {
		this(note, longnote, minenote, null, null);
	}

	public SkinNote(SkinSource[] note, SkinSource[][] longnote, SkinSource[] minenote, SkinSource[] hidden, SkinSource[] processed) {
		lanes = new SkinLane[note.length];
		for(int i = 0;i < lanes.length;i++) {
			// 获取对应 lane 的 hidden 和 processed，如果有的话
			SkinSource hiddenNote = (hidden != null && i < hidden.length) ? hidden[i] : null;
			SkinSource processedNote = (processed != null && i < processed.length) ? processed[i] : null;
			lanes[i] = new SkinLane(note[i], longnote[i], minenote[i], hiddenNote, processedNote);
		}

        // 重要：必须设置至少一个 destination，否则会在 validate() 阶段被删除
	...
	public void prepare(long time, MainState state) {
		...
		// 确保当 renderer 可用时，draw 字段设置为 true
		draw = true;
		this.time = time;
