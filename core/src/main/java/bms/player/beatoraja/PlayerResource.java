...
public final class PlayerResource {
	...
	private FileHandle[] bmsFileHandles;
...
//		rscore = null;
	...
	public boolean setBMSFile(final FileHandle f, BMSPlayerMode mode) {
		// TODO play mode, リプレイデータでの読み込み分岐をここで行う
		this.mode = mode;
		replay = new ReplayData();
		model = loadBMSModel(f, pconfig.getLnmode());
		if (model == null) {
			Logger.getGlobal().warning("楽曲が存在しないか、解析時にエラーが発生しました:" + f.path());
			return false;
		}
	...
	public BMSModel loadBMSModel(FileHandle f, int lnmode) {
	...
	public BMSModel loadBMSModel(int[] selectedRandom) {
		if(model != null) {
			...
			if (info.fileHandle != null) {
				return loadBMSModel(new ChartInformation(info.fileHandle, info.lntype, selectedRandom));
			} else {
	...
	public BMSModel loadBMSModel(ChartInformation info) {
		ChartDecoder decoder = info.fileHandle != null ? ChartDecoder.getDecoder(info.fileHandle) : ChartDecoder.getDecoder(info.path);
		if(decoder == null) {
			return null;
		}
	...
	public boolean setCourseBMSFiles(FileHandle[] files) {
	...
	public void setAutoPlaySongs(FileHandle[] paths, boolean loop) {
		this.bmsFileHandles = paths;
		this.loop = loop;
	}

	public boolean nextSong() {
		if(bmsFileHandles == null) {
			return false;
		}
		final int orgindex = courseindex;
		do {
			if(courseindex == bmsFileHandles.length) {
				if(loop) {
					courseindex = 0;
				} else {
					return false;
			...
			songdata = null;
			if(setBMSFile(bmsFileHandles[courseindex++], BMSPlayerMode.AUTOPLAY)) {
				return true;
			};
	...
	public boolean nextCourse() {
		...
		if (courseindex == course.length) {
			...
			songdata = null;
			setBMSFile(Gdx.files.absolute(course[courseindex].getPath()), mode);
			return true;
		}
	}
	...
	public void reloadBMSFile() {
		if (model != null) {
			ChartInformation info = model.getChartInformation();
			if (info.fileHandle != null) {
				model = loadBMSModel(info.fileHandle, pconfig.getLnmode());
			} else {
				model = loadBMSModel(Gdx.files.absolute(model.getPath()), pconfig.getLnmode());
			}
		}
