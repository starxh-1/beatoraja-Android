...
public class BMSONDecoder extends ChartDecoder {
	...
	public BMSModel decode(FileHandle f) {
		Logger.getGlobal().fine("BMSONファイル解析開始 :" + f.path());
		log.clear();
		tlcache.clear();
		...
		try {
			...
			try (InputStream is = new DigestInputStream(new BufferedInputStream(f.read()), digest)) {
				bmson = mapper.readValue(is, Bmson.class);
			}
	...
	private BMSModel decodeBmson(Bmson bmson, String path, long currnttime, FileHandle f) {
		...
		Logger.getGlobal().fine("BMSONファイル解析完了 :" + path + " - TimeLine数:" + tlcache.size() + " 時間(ms):"
